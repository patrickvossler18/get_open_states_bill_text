    ...
    for name in sub_dir_names:
        ...
        for house in ['lower', 'upper']:
            process = subprocess.Popen(
                "sudo bash /Users/patrick/Downloads/Dropbox-Uploader/dropbox_uploader.sh list  Bill\ datasets\ fr\ openstates\ 2014_2017/" +
        ...
        uploaded_files = list(filter(
            lambda path: (path.endswith(".pdf") or path.endswith(
                ".html") or path.endswith(".htm") or path.endswith(".doc") or path.endswith(".txt")) and
                         (not bool(re.search('[A-Z]+ [0-9]$', path.strip()))) and (not path.endswith(".json")) and path not in finished_path_upload,
            uploaded_files))
    space = False
    for dir in sub_dir:
        if " " in dir:
...
for state in states:
    ...
    pool = Pool(6)  # go fast the first time
